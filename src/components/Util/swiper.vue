<template>
  <section class="wh_content" @touchmove="fn">
    ...
    <div v-if="showIndicator" class="wh_indicator">
      <div v-for="(tag,$index) in slidesLength" :class="{ wh_show_bgcolor: index-1==$index }" :key="$index" class="wh_indicator_item"/>
    </div>

    <div v-if="slidesLength > 1 && index < slidesLength" :style="{backgroundImage: `url(${NextIconUrl})`}" class="wh_next" @click="nextSlide"/>

    <div v-if="slidesLength > 1 && index > 1" :style="{backgroundImage: `url(${PrevIconUrl})`}" class="wh_prev" @click="prevSlide"/>
  </section>
</template>
